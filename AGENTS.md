...
Вход в браузере использует одно поле токена. HTTP-адаптер проверяет Origin и
не возвращает токен в JSON. `scripts/prepare-owner-signin.mjs` подключает
OwnerAuth к штатной форме Factory 0.14.0 после сборки; при изменении ожидаемого
bundle сборка должна остановиться до проверки совместимости новой версии.
